...
void OnecSPI(){
  ...
  digitalWrite(SS, LOW);      //このSlaveをセレクトする//（マスタ通信は有効です）
  
  SPI.transfer(0);            // 1byte送受信 //値は受け取らない
  
  delayMicroseconds(25);      //Slave割り込みの計算完了待ち
  rcv_data.rcv[0] = SPI.transfer(1);   // 1byte送受信 //値を受け取る
  delayMicroseconds(25);     //Slave割り込みの計算完了待ち
  rcv_data.rcv[1] = SPI.transfer(2); // 1byte送受信 //値を受け取る
  delayMicroseconds(25);     //Slave割り込みの計算完了待ち
  rcv_data.rcv[2] = SPI.transfer(3);   // 1byte送受信 //値を受け取る
  delayMicroseconds(25);     //Slave割り込みの計算完了待ち
  rcv_data.rcv[3] = SPI.transfer(255); // 1byte送受信 //値を受け取る
